import { addTarjeta } from ".."; // Importas la acción que añade el producto al estado
import { fetchAsync } from "../../../../utils/FetchAsync"; // Importas una función auxiliar para hacer la petición asincrónica

export const createTarjeta = (nuevotarjeta) => async (dispatch) => {
    ...
    try {
        ...
        const nuevoTarjetaCreada = await fetchAsync('POST',`${url}tarjeta`, nuevotarjeta); // Haces la solicitud POST y esperas la respuesta
        
        // 2. Una vez que se resuelve la petición, despachas la acción addProducto
        dispatch(addTarjeta(nuevoTarjetaCreada)); // En este paso, se actualiza el estado en el Store con el nuevo producto
